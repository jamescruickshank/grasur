...
#
fns = os.listdir('sixdata/')

l = []
for fn in fns:
    if fn.startswith('XXX')==False:
        print("loading data from %s"%fn)
        with open("sixdata/%s"%fn) as datafile:
...
fnsb = [x for x in os.listdir('../six/') if x.startswith('ors')]

k = []
for fn in fnsb:
    print("loading data from %s"%fn)
    with open("../six/%s"%fn) as datafile:
        data = json.load(datafile)
    for x in data:
